...
public class PhotoSelectorActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().hasExtra("photos")) {
			selected = (ArrayList)getIntent().getSerializableExtra("photos");
		}
		if (selected==null) {
		...
		layout_toolbar_ar = (RelativeLayout) findViewById(R.id.layout_toolbar_ar);
	...
	public void onClick(View v) {
		if (v.getId() == R.id.btn_right_lh)
			ok(); // 选完照片
		else if (v.getId() == R.id.reviewlayout)
			//album();
			finish();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == REQUEST_CAMERA && resultCode == RESULT_OK) {
			...
			// selected photos
			// ///////////////////////////////////////////////////////////////////////////////////////////
			System.out.println( "======= onActivityResult  selected.size(): "+selected.size()+" size: "+selected.size());
			...
			if (selected.size() >= MAX_IMAGE) {
				...
				photoModel.setChecked(false, "onActivityResult");
	...
	/** 完成 */
	private void ok() {
		System.out.println( "======= okokokokok  selected.size(): " + selected.size() + " size: " + selected.size());
	...
	private void popAlbum() {
		tvTitle.setText("照片");
		layout_toolbar_ar.setVisibility(View.GONE);
...
//				.setLinearInterpolator().startAnimation(layoutAlbum);
	...
	private void hideAlbum() {
		...
		layout_toolbar_ar.setVisibility(View.VISIBLE);
...
//				.setLinearInterpolator().startAnimation(layoutAlbum);
	...
	public void onCheckedChanged(PhotoModel photoModel,
		...
		System.out.println("======= onCheckedChanged  isChecked: " + isChecked + " size: " + selected.size());
		photoAdapter.setSelectedSize(selected.size());
		if (selected.size()+1>MAX_IMAGE) {
			if (isChecked) {
				Toast.makeText(
						this,
						String.format(
			...
			photoModel.setChecked(false, "PASonCheckedChanged");
			if (selected.contains(photoModel)){
		...
		public void onPhotoLoaded(List<PhotoModel> photos) {
			for (PhotoModel model : photos) {
				if (selected.contains(model)) {
					model.setChecked(true,"PhtoSelectorAcitivyt-OnLocalReccentListener");
				}
			}
